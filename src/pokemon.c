...
void CalculateMonStatsNoHeal(struct Pokemon *mon)
{
    s32 oldMaxHP = GetMonData(mon, MON_DATA_MAX_HP, NULL);
    s32 currentHP = GetMonData(mon, MON_DATA_HP, NULL);
    s32 hpIV = GetMonData(mon, MON_DATA_HP_IV, NULL);
...
void BoxMonToMon(const struct BoxPokemon *src, struct Pokemon *dest)
{
    ...
    {
        u16 storedhp = src->unknown;   // stored HP
        u16 maxHP = GetMonData(dest, MON_DATA_MAX_HP, NULL);
        if (storedhp > maxHP)
            storedhp = maxHP;

        SetMonData(dest, MON_DATA_HP, &storedhp);
        u8 nv = src->unused & 0x07;  // read only lowest 3 bits
        u32 status = 0;
        switch (nv)
        {
            case 1: status = STATUS1_POISON;     break;
            case 2: status = STATUS1_BURN;       break;
