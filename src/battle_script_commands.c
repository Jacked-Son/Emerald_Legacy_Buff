                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    if ((levelCappedNuzlocke(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL))))
                    {
                        if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) < getLevelCap())
                            ...
                            gBattleMoveDamage = ((gBattleMoveDamage * 10) / 100) + 1;    // 1 above cap
                        else if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) == getLevelCap() + 2)
                             gBattleMoveDamage = ((gBattleMoveDamage * 5) / 100) + 1;    // 2 above cap
                        else
                         gBattleMoveDamage = ((gBattleMoveDamage * 1) / 100) + 1;    // 3+ above cap
                    }
